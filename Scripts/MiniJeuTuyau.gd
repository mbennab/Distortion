...
const ROUND_COLS := [5, 6, 8]
const ROUND_ROWS := [4, 4, 5]
const TILE_SIZE := 72
const TILE_GAP := 4
...
func _init_round(round_num: int) -> void:
	...
	grid_cols = ROUND_COLS[round_num - 1]
	grid_rows = ROUND_ROWS[round_num - 1]
...
func _generate_puzzle() -> void:
	...
	for i in range(path.size()):
		...
		if i == 0:
			connections.append(Side.LEFT)
		if i == path.size() - 1:
			connections.append(Side.RIGHT)

		if i > 0:
			var prev: Vector2i = path[i - 1]
			var diff := prev - cell
			connections.append(_diff_to_side(diff))
		...
		var pipe := _find_pipe_for_connections(connections)
...
func _bfs_from_source() -> Dictionary:
	...
	var drain_cell: Dictionary = grid[_drain_row][grid_cols - 1]
	...
	var reaches := visited.has(_v2key(Vector2i(grid_cols - 1, _drain_row))) and has_right
...
func _build_ui() -> void:
	...
	var drain_y := _grid_origin.y + _drain_row * (TILE_SIZE + TILE_GAP) + TILE_SIZE / 2.0
	...
	drain_glow.position = Vector2(_grid_origin.x + grid_w + 2, drain_y - 18)
	...
	drain_label.position = Vector2(_grid_origin.x + grid_w + 16, drain_y - 10)
...
func _show_leak() -> void:
	...
	var leak_x := _grid_origin.x + (grid_cols - 1) * (TILE_SIZE + TILE_GAP) + TILE_SIZE / 2.0
	var leak_y := _grid_origin.y + _drain_row * (TILE_SIZE + TILE_GAP) + TILE_SIZE / 2.0
	...
	var drain_cell_bg: ColorRect = _cell_bg[_drain_row][grid_cols - 1]
	...
	tween.tween_callback(_on_leak_stop_sparks.bind(spark1, spark2))
	tween.tween_callback(_on_leak_message)
	tween.tween_interval(3.0)
	tween.tween_callback(_on_leak_finish)


func _on_leak_stop_sparks(s1: CPUParticles2D, s2: CPUParticles2D) -> void:
	s1.emitting = false
	s2.emitting = false
